...
from msaf.lib.tools.export import export_arlequin
...
class ArlequinResult(object):
    ...
    def __init__( self, output, error = None, sets = None ):
        self.output = output
        self.error = error
        self.sets = sets
        self.labels = {}
...
def standardized_fst( fst_orig, fst_max ):
    ...
    if (fst_orig.sets != fst_max.sets) or (fst_orig.labels != fst_max.labels):
        raise RuntimeError( 'FST original and maximum are not in the same dataset' )

    fst_std = ArlequinResult( None )
    fst_std.labels = fst_orig.labels
    fst_std.sets = fst_orig.sets
...
def arlequin( querytexts, threshold, markers ):
    """ function called from web interface """
    pass
