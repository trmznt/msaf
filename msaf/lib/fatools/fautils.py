...
def scan_ladder_peaks( channel, ladders, parameter = None ):
    ...
    # only use peak-scanned
    for p in initial_peaks:
        channel.alleles.append(p)
    initial_peaks = [ p for p in initial_peaks if p.type == 'peak-scanned' ]
...
def scan_peaks( channel, parameter=None ):
    ...
    for peak in peaks:
        ...
        if parameter.ignoredpeaks and is_ignored( allele.rtime, parameter.ignoredpeaks ):
            allele.type = 'peak-noise'
            #continue
        elif allele.beta > parameter.max_beta:
            allele.type = 'peak-broad'
            #continue
...
def is_ignored(peak, peak_list):
    for ignored_peak in peak_list:
        if ignored_peak - 5 < peak < ignored_peak + 5:
            return True
    return False
